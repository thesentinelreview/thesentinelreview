...
export default function TheatersError({
  error,
  unstable_retry,
}: {
  ...
  useEffect(() => {
    console.error("[/theaters] render error:", error);
  }, [error]);
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-lg font-bold text-slate-100 mb-1">
              Theaters temporarily unavailable
            </h2>
            <p className="text-sm text-slate-400 leading-relaxed">
              The theaters index could not be rendered right now. Try again in a moment.
            </p>
            {error?.digest && (
