...
that the closest returned candidate is selected. They also assert the precision
gate: a coarse-precision incoming event skips dedup entirely.
"""
...
class TestFindDuplicate:
    ...
    def test_returns_single_candidate_as_row_dict(self) -> None:
        ...
        # merge with the matched event's occurred_at + distance + precision.
        assert result == candidate
        assert result is not None
        assert result["id"] == candidate["id"]
        assert "occurred_at" in result and "dist_km" in result and "geocode_precision" in result
    ...
    def test_coarse_incoming_skips_dedup_without_querying(self) -> None:
        # A region/country/unknown-precision event sits on a centroid, not a real
        # location, so it can't be spatially deduped — return None and never query.
        for precision in ("region", "country", "unknown"):
            conn = _mock_conn()
            with patch("sentinel.pipeline.dedup.find_nearby_events") as mock_fn:
                result = find_duplicate(
                    conn, lng=49.0, lat=31.0, occurred_at=_OCC, event_type="strike",
                    incoming_precision=precision,
                )
            assert result is None, precision
            mock_fn.assert_not_called()

    def test_city_incoming_queries_candidates(self) -> None:
        candidate = _candidate(dist_km=1.0)
        conn = _mock_conn()
        with patch(
