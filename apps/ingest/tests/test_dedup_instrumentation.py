...
best-effort writer for the dedup_decisions audit trail (migration 0024).

Verifies it computes the merge gap/distance, records NULLs for a fresh event, and
— critically — never lets an instrumentation failure abort ingest. conn is mocked.
"""
...
def test_merge_records_gap_and_distance() -> None:
    conn = MagicMock()
    matched = {"id": uuid.uuid4(), "occurred_at": _OCC, "dist_km": 2.0}
...
def test_new_records_null_match_fields() -> None:
    conn = MagicMock()
    with patch("sentinel.jobs.extract_events.record_dedup_decision") as rec:
        _record_dedup_decision_safe(
            conn,
